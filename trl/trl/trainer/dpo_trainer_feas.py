...
class DPOfTrainer(Trainer):
    ...
    def prediction_step(
        ...
        return (loss.detach(), logits, labels, all_losses.detach())
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            _, logits, labels, loss = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            ...
            if loss is not None:
                losses = self.gather_function(loss)
        ...
        if all_losses is not None:
            metrics[f"{metric_key_prefix}_loss/mean"] = all_losses.mean().item()
